...
-   Les codes R se trouvent dans les menus déroulants sous l'onglet " 👈 **Voir le code ici**"
...
Ce tableau contient toutes les données recueillie par les 6 équipes du groupe A et B. Ensemble, nous avons échantillonné 10 lacs différents, pour un total de 66 observations. 👏
...
Grâce a cette carte, vous pouvez identifier vos sites d'échantillonnage et repérer ceux de vos collègues également. Les équipes du groupe A sont identifiées avec les cercles mauves, alors que celles du groupe B le sont avec des cercles oranges.
...
# Diagramme à moustache \| Conductivité l'eau

La conductivité de l'eau représente sa capacité a conduire le courant électrique. Elle est directement liér à la quantité d'ions dissous dans l'eau. L'épandage de sel sur les routes en hiver créer un apport élevé en sel (ions Na+ et Cl-) dans les eaux de ruissellement qui finissent par se jeter dans les lacs. La conductivité de l'eau était en moyenne 5 fois plus elevée dans les lacs à l'extérieur qu'à l'intérieur de la SBL.
...
Le ratio de drainage (aire du bassin versant / aire du lac) ne semble pas avoir d'impact sur la conductivité de l'eau pour les lacs dans la SBL, mais il semble être négativement corrélé a celui de la conductivité dans les lacs hors de la SBL. Un lac avec un grand ratio de drainage a un grand bassin versant par rapport à sa superficie. Par exemple, on observe que le lac rond avait la conductivité la plus élevée, autant lors de l'échantillonnage du groupe A que B.
...
Malgré quelques difficultés techniques lors de la deuxième semaine, les données des chambres flottantes indiquent que certains lacs étaient bel et bien de grandes sources de CO₂ vers l'atmosphère (p.ex Triton) alors que d'autres étaient de faibles puits de CO₂ (p.ex Croche et Connely)
...
cleveland_dotplot=ggplot() +
  ...
  geom_point(data=data%>% filter(GROUPE == "A"), 
             aes(x=NOM_DU_LAC, y=Flux_CO2, label=ÉQUIPE), 
             color="purple", size=3 ) +
...
![](Terrain 1.2.png){fig-align="center" width="350"}

Merci de nous avoir lu et pour votre participation au cours. Nous vous souhaitons une bonne continuité dans vos étude.

Au plaisir de se croiser sur le campus!
